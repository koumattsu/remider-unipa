# backend/test/test_outcome_analytics_summary_contract.py
...
def test_outcome_analytics_summary_contract(client):
    ...
    if items:
        it0 = items[0]
        assert set(it0.keys()) == {"period_start", "period_end", "total", "done", "missed", "done_rate"}
        assert isinstance(it0["period_end"], str)
